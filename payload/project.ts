      ...
      descriptions: [
        {
          ...
          descriptions: [
            {
              content:
                '[DB 성능 개선] 프로시저 전면 수정 및 CTE 활용 쿼리 최적화로 **배치/조회 속도 약 30% 향상**',
            },
            {
              content:
                '[UI/UX 개발] 불량 데이터 추적 차트 및 통계 보고서 신규 개발로 **검증 시간 83% 단축(일 2시간 → 20분)**',
            },
            {
        ...
        {
          content:
            '**MVC 기반 프론트 컨트롤러 도입**을 통한 로깅(Logging) 모듈 공통화 및 구조 개선',
          descriptions: [
            {
              content:
                '[아키텍처 개선] **프론트 컨트롤러 패턴을 설계 및 도입**하여 모든 요청(Request)의 진입점 통합',
            },
            {
              content:
                '[공통 모듈화] 로깅 기능을 프론트 컨트롤러로 분리(Decoupling)하여 **표준화된 로그 일괄 적재** 구현',
            },
            {
              content:
                '보일러플레이트 코드 제거를 통해 **코드 라인 수(LOC) 약 50% 감소** 및 핵심 로직 가독성 극대화',
            },
          ],
        },
        {
          ...
          descriptions: [
            {
              content:
                '데이터 통신 표준을 XML에서 JSON 방식으로 전면 전환하여 **복잡한 파싱 로직 제거**',
            },
            {
            ...
            {
              content:
                '네트워크 페이로드 감소로 **웹 응답 속도 15~30% 개선** 및 신규 API **개발 리드타임 단축**',
            },
          ],
        },
        {
